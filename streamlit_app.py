...
st.set_page_config(page_title="🧺 Fashion Chatbot Dataset Augmentor", layout="wide")
...
def safe_read_csv(uploaded_file, name):
    try:
        ...
        if df.empty:
            st.warning(f"⚠️ The {name} file is empty.")
        return df
    except Exception as e:
        st.error(f"❌ Failed to read {name}. Error: {str(e)}")
        try:
            raw_text = uploaded_file.read().decode('utf-8')
            st.text_area(f"📄 Raw content of {name}:", raw_text, height=200)
        except:
            st.error(f"❌ Also failed to display raw content of {name}.")
        return None

if styles_file and chatbot_file:
    df_styles = safe_read_csv(styles_file, "styles.csv")
    chatbot_df = safe_read_csv(chatbot_file, "ClothesShopChatbotDataset.csv")

    if df_styles is not None and chatbot_df is not None:
        # Clean and display styles
        ...
        st.dataframe(chatbot_augmented.tail(15))
    ...
    st.info("📂 Please upload both `styles.csv` and `ClothesShopChatbotDataset.csv` to proceed.")
